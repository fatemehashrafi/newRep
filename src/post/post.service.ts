...
import { EventEntity, EventTypes } from 'src/event/entities/event.entities';
...
export class PostService {
  constructor(
    ...
    this.loggerService.log(`constructor, Currency Sign is: ${this.currencySign}`,);
    this.loggerService.log(`SHOW_MOBILE_NUMBER :${this.postconfig.showMobileNumber}`)
  }

  findAll(pagination?: PaginationDto) {
    this.loggerService.setPrefix('findAll coled');
  ...
  async delete(id: number) {
    ...
    if(!post){
      throw new NotFoundException(`post whit id of ${id} not found`);
    }
